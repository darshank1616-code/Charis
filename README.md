...
[Charis.html](https://github.com/user-attachments/files/28681140/Charis.html)
```html.HBD
<!DOCTYPE html>
<html lang="en">
<head>
...
<body class="crt min-h-screen flex flex-col justify-between retro-grid relative selection:bg-[#ff2a93] selection:text-white p-4" onclick="initAudioContextOnInteraction()">
    ...
    <div id="stage-preloader" class="fixed inset-0 z-[9999] bg-[#1a0b16] flex flex-col items-center justify-center p-4">
        ...
        <div class="z-10 text-center max-w-xl w-full">
            ...
            <div class="flex items-center gap-4 mb-6 bg-[#2d1226]/60 p-2 border-2 border-[#ff2a93] rounded">
                <div class="w-full h-8 bg-[#1a0b16] border-2 border-[#ff7ebd] p-1 flex">
                    <div id="preloader-progress" class="h-full bg-[#ff2a93] w-0"></div>
                </div>
                <div class="vt323-text text-3xl font-bold text-[#ffdf7e] w-12 text-right" id="preloader-counter">7s</div>
            </div>
    ...
    <main class="flex-grow flex items-center justify-center p-4 z-10">
        ...
        <div id="stage-transition" class="hidden fixed inset-0 z-50 bg-[#1a0b16] flex-col items-center justify-center">
            ...
            <div class="text-center">
                ...
                <div class="w-64 h-8 bg-[#2d1226] border-4 border-[#ff7ebd] p-1 flex">
        ...
        <!-- STAGE 4: Strawberry Captcha (Updated to Best of 3) -->
        ...
        <div id="stage-mole" class="stage-div hidden w-full max-w-xl bg-[#2d1226]/90 p-5 md:p-8 retro-border text-center rounded-sm">
            ...
            <!-- Controls (Unlocked at 10 Moles) -->
            <div id="mole-control-panel" class="hidden flex flex-col sm:flex-row gap-4 justify-center items-center pt-4 border-t-4 border-[#ff2a93]/50">
                <button onclick="restartMoleGame()" class="retro-button w-full sm:w-40 bg-[#1a0b16] text-[#ff7ebd] font-bold py-3 px-4 hover:bg-[#2d1226] text-xs">
                    [ PLAY AGAIN ]
                </button>
                <button onclick="handleMoleSuccess()" class="retro-button w-full sm:w-40 bg-[#ff2a93] text-white font-bold py-3 px-4 hover:bg-[#ff55a9] text-xs">
                    [ NEXT ]
                </button>
            </div>
        ...
        <!-- STAGE PUNCH: Arcade Punching Machine -->
        <div id="stage-punch" class="stage-div hidden w-full max-w-xl bg-[#2d1226]/90 p-5 md:p-8 retro-border text-center rounded-sm">
            <div class="mb-1 text-[#ff7ebd] text-xs uppercase tracking-widest vt323-text text-xl">CHALLENGE: MINI-GAME 02</div>
            <h1 class="text-base md:text-xl font-bold mb-2 text-white glitch">ARCADE PUNCH MACHINE</h1>
            <p class="vt323-text text-lg text-[#ff9ed2] mb-4">Time the moving gauges! Score <span class="text-[#ffdf7e] font-bold">over 850</span> to advance!</p>
            ...
            <div class="flex flex-col justify-center items-center gap-4">
                ...
                <!-- Controls revealed upon passing score criterion (>850) -->
                <div id="punch-success-panel" class="hidden flex flex-col sm:flex-row gap-4 w-full justify-center mt-2 border-t-4 border-[#ff2a93]/30 pt-4">
                    <button onclick="resetPunchMachine()" class="retro-button w-full sm:w-40 bg-[#1a0b16] text-[#ff7ebd] font-bold py-3 px-4 text-xs">
                        [ REPLAY ]
                    </button>
                    <button onclick="goToStageTuff()" class="retro-button w-full sm:w-40 bg-[#7effa1] text-black font-bold py-3 px-4 hover:bg-[#a3ffbc] text-xs">
                        [ NEXT ]
                    </button>
                </div>

                <!-- Try again response block -->
    ...
    <script>
        ...
        // PRETTIER 7-SECOND STARTUP SCREEN WITH COUNTDOWN
        ...
        function startMoleGame() {
            ...
            moleSpawnInterval = setInterval(() => { if (moleGameActive) spawnMole(); }, 850);
        }
        ...
        function spawnMole() {
            if (activeMoleId) {
                ...
                prevMole.querySelector('.normal-face').classList.remove('hidden');
                prevMole.querySelector('.whacked-face').classList.add('hidden');
            }
        ...
        function endMoleGame(success) {
            ...
            clearInterval(moleTimerInterval); clearInterval(moleSpawnInterval);
            if (success) { playScoreFeedback(true); document.getElementById('mole-control-panel').classList.remove('hidden'); }
            else { playSound('fail'); document.getElementById('mole-fail-panel').classList.remove('hidden'); }
        }
        ...
        function triggerPunchAction() {
            ...
            if (activeAxis === 'X') {
                ...
                if (finalCalculatedScore >= 850) {
                    ...
                    document.getElementById('punch-btn').className = "hidden";
